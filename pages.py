...
class EndBlock(Page):
    ...
    def vars_for_template(self):
        ...
        while (i < self.subsession.config.get('block_total')):
            ...
            i += 1
        return{
            'block_num': int(self.subsession.config.get('round')/self.subsession.config.get('block_total')),
...
class payment_page(Page):
    ...
    def vars_for_template(self):
        ...
        return{
            ...
            'total_payoff': round((payment_payoff - participation_fee_total)*.06,2)
        }
